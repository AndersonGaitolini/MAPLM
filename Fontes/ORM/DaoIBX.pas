...
uses Data.Db, Base, Rtti, Atributos, system.SysUtils, system.Classes,
     system.Generics.Collections,

  ibx.IBQuery, ibx.IBDatabase,
  ibx.IB, IBX.IBHeader, IBX.IBExternals, IBX.IBXConst, IBX.IBIntf;
...
procedure TQueryIBX.SetParamBoolean(AProp: TRttiProperty; ACampo: string;
...
procedure TQueryIBX.SetParamDateTime(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
 inherited;
  if AProp.GetValue(ATabela).AsType<TDateTime> = 0 then
...
procedure TQueryIBX.SetParamFileStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
...
begin
  ...
  begin
    ...
    TIBQuery(AQry).ParamByName(ACampo).LoadFromStream(stream, ftBlob);
  end
  else
  TIBQuery(AQry).ParamByName(ACampo).AsStream := nil;
end;
...
procedure TQueryIBX.SetParamStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
var stream: Tstream;
begin
...
procedure TQueryIBX.SetCamposBoolean(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  TIBQuery(AQry).ParamByName(ACampo).AsBoolean := AProp.GetValue(ATabela).AsBoolean;
end;
...
procedure TQueryIBX.SetCamposDateTime(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TIBQuery(AQry).FieldByName(ACampo).AsDateTime);
end;

procedure TQueryIBX.SetCamposFileStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TIBQuery(AQry).CreateBlobStream(TIBQuery(AQry).FieldByName(ACampo),bmWrite));
end;
...
procedure TQueryIBX.SetCamposStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TIBQuery(AQry).CreateBlobStream(TIBQuery(AQry).FieldByName(ACampo),bmWrite));
end;
...
function TDaoIBX.ConsultaAll(ATabela: TTabela; AOrderBy: string = ''): TDataSet;
var
  AQry: TIBQuery;
begin
